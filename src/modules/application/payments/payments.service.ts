...
export class PaymentsService {
  ...
  public async createPreferences(body: CreatePreferencesDto) {
    this.logger.log(
      chalk.magenta('🚀 Iniciando creación de preferencia de pago'),
    );
    try {
      if (body.payer) {
        ...
        if (needsUpdate) {
          ...
          this.logger.log(
            chalk.green('📦 Usuario actualizado con nueva información:') +
              '\n' +
              chalk.cyan(
                JSON.stringify(
      ...
      this.logger.log(
        chalk.green('✅ Preferencia creada exitosamente: ') +
          chalk.yellow(preferences.id),
      );
      return preferences;
    } catch (error) {
      this.logger.error(
        chalk.red('❌ Error al crear preferencia: ') +
  ...
  public async registerPayment(payment: RegisterPaymentDto) {
    const result = await this.mercadopagoService.getPaymentStatus(
      payment.reference,
    );
    ...
    if (result && (result as any).status === 'approved') {
      ...
      this.logger.log(
        `✅ Pago registrado en la base de datos: ${paymentDB.id}`,
      );

      return { message: 'Pago registrado exitosamente', details: result };
    } else {
      this.logger.log(
        `⚠️ Pago no aprobado para la referencia: ${payment.reference} - Estado: ${(result as any)?.status || 'unknown'}`,
      );
